              ...
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono text-gray-400">{new Date(record.timestamp).toLocaleDateString()}</span>
                    <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 text-xs rounded">
                      {record.problem.subject}
                    </span>
                  </div>
                  <button onClick={() => onDelete(record.id)} className="text-gray-300 hover:text-red-500">
